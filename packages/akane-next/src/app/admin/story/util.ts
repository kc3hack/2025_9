...
export const getHintFromFormData = (data: FormData) => {
  return {
    storyID: BigInt(data.get("question-story-id")?.toString() ?? "0"),
    title: data.get("hint-title")?.toString() ?? "",
    content: data.get("hint-content")?.toString() ?? "",
    image_url: data.get("hint-image_url")?.toString() ?? null,
  };
};
