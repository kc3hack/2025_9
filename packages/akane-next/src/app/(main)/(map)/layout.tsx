...
export default function MapLayout({
	...
	const { position, isFirstFlyLocation, setIsFirstFlyLocation } = useLocation(); // useLocationフックを使用して位置情報を取得
	const [mapCenter, setMapCenter] = useState<[number, number]>(
		position
			? [position.coords.longitude, position.coords.latitude]
			: [135.739078, 34.995022],
	);
	const { map, mapContainerRef, isMapLoaded, followMode } = useMap();
